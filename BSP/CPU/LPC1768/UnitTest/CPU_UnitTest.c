...
/*
 * Test CPU_Halt function.
 *  When this function is called system should be broken.
 *   - Interrupts disabled.
 *   - Blocking in a endless while loop.
 */
void test_CPU_Halt(void)
{
	CPU_Halt();

	/* CPU_Halt disable interrupts and enters to */
...
/*
 * Tests Functionality whicj starts Context Switching
 */
void test_CPU_CS_Start(void)
{
	...
	/* Check for internal global variables which keeps next task (TCB)*/
	TEST_ASSERT((currentTask == &initialTCB));
	TEST_ASSERT((nextTask == &initialTCB));
...
void test_CPU_CS_YieldTo(void)
{
	...
	/* provided TCB should be kept in nextTask object for next context switching */
	TEST_ASSERT((nextTask == &newTCB));
	 ...
	 */
	TEST_ASSERT((currentTask == &newTCB));
}
...
void test_CPU_CS_InitializeTaskStack(void)
{
	...
	/* Check R0 register. We dont pass no argument so should be zero. */
...
/*
 * Tests address alignment of a Stack
 */
